...
/*
 * ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 *  🤖 ZYREX — AI GROUP PERSONA (v3)
 * ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 * Changes from v2:
 *   - FIXED: Zyrex no longer fires on prefixed commands
 *     (.ping, .antilink, etc.) even mid-session — those
 *     always go to the real bot only, never both.
 *   - Sticker system now reads from local folders instead of
 *     URL arrays: cmds/zyrex_stickers/<mood>/*.webp|png|jpg
 *     Drop files in, no code changes needed, no restart
 *     required (folder is read fresh each send).
 *   - Tier 2 confirmation prompts shortened + varied per
 *     action instead of one repeated template.
 *   - Personality overhaul: "lazy genius" instead of generic
 *     sarcastic. Naija slang (kept current, non-cringe).
 *   - Gender-aware softness: if a girl calls Zyrex mean/rude,
 *     it checks/asks and eases up; stays unapologetic with guys.
 *   - Opinions: likes garri (a lot), has a few running
 *     preferences instead of reacting identically to everything.
 *   - Lightweight "annoyance memory" per user per chat: if
 *     someone's been repeatedly rude/annoying in the current
 *     session, Zyrex's tone toward them shifts drier/shorter.
 *
 * Requires GROQ_API_KEY env var (Render dashboard).
 * Uses node-fetch + wa-sticker-formatter (existing deps).
 *
 * Safety guardrails unchanged and non-negotiable — see v2
 * comment block for the full list (age, sexual content,
 * slurs, harassment, honesty about being AI, Tier-2 action
 * confirmation rules).
 */
...
const ZX = {
  ...
  // Annoyance memory: how many "rude" flags in a session before tone shifts
  ANNOYANCE_THRESHOLD: 3,
  ANNOYANCE_DECAY_MS: 20 * 60 * 1000, // forget grudges after this long idle
}

// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
//  STICKERS — folder based. Drop files into:
//    cmds/zyrex_stickers/<mood>/*.webp|.png|.jpg
//  Moods: savage, laughing, done, shock, smug, neutral
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
const ZYREX_STICKER_DIR = pathZyrex.join(__dirname, "zyrex_stickers")
const ZYREX_VALID_EXT = new Set([".webp", ".png", ".jpg", ".jpeg"])

function zyrexPickStickerFile(mood) {
  try {
    const tryDirs = [mood, "neutral"]
    for (const dir of tryDirs) {
      const fullDir = pathZyrex.join(ZYREX_STICKER_DIR, dir)
      if (!fsZyrex.existsSync(fullDir)) continue
      const files = fsZyrex.readdirSync(fullDir).filter(f =>
        ZYREX_VALID_EXT.has(pathZyrex.extname(f).toLowerCase())
      )
      if (files.length) {
        const pick = files[Math.floor(Math.random() * files.length)]
        return pathZyrex.join(fullDir, pick)
      }
    }
  } catch (e) {
...
async function zyrexSendSticker(m, chatJid, mood) {
  try {
    const filePath = zyrexPickStickerFile(mood)
    if (!filePath) return
    const buffer = fsZyrex.readFileSync(filePath)
...
if (!global.zyrexAnnoyance) global.zyrexAnnoyance = new Map() // "chat_user" -> { count, lastAt, genderKnown }
...
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
//  ANNOYANCE MEMORY — lightweight, per chat+user, in-memory
//  only (resets on restart, which is fine — this is tone
//  flavor, not a real record of anyone).
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
// Very light heuristic — this only affects TONE, never any real action.
...
//  Prompts are per-action and short, not one repeated template.
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
const ZYREX_CONFIRM_PROMPTS = {
  ...
  mute: () => `lock the group? confirm "yes", 60s`,
  unmute: () => `open the group back up? "yes" to confirm, 60s`,
}
...
Sticker packs live in cmds/zyrex_stickers/<mood>/
just drop image files in, no restart needed.\`\`\``
    )
  } catch (e) {
...
kord({ on: "all" }, async (m, text) => {
  try {
    ...
    // ── NEVER intercept real bot commands, even mid-session ──
    // prefix is a regex-style string like "[.!?]" per config.env;
    // this checks if the message starts with ANY configured prefix char.
    const prefixChars = (prefix || ".").replace(/[\[\]]/g, "")
    if (prefixChars.split("").some(ch => text.startsWith(ch))) return
    ...
    if (justFlooded) {
      await m.client.sendMessage(chatJid, {
        text: "y'all are a lot right now, taking a few — back shortly",
      })
      return
    }
    ...
    // ── Annoyance tracking (tone only, never a real moderation input) ──
    if (zyrexTextSeemsHostile(text)) zyrexBumpAnnoyance(chatJid, userJid)
    ...
    const messages = [
      { role: "system", content: zyrexSystemPrompt({ isRequesterOwner, senderName, dailyHeadsUp, annoyanceLevel: annoyance.count || 0 }) },
      ...session.history,
    ]
