...
/*
 * ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 *  🤖 ZYREX — AI GROUP PERSONA
 * ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 * A conversational AI persona that can be enabled per-group.
 * Uses Groq's API (OpenAI-compatible /chat/completions) for
 * fast, free-tier LLM replies. Requires GROQ_API_KEY env var
 * (set in Render dashboard, never committed to code).
 *
 * Reply triggers (in order):
 *   1. Reply to one of Zyrex's own messages
 *   2. Message contains the bot's name ("zyrex", case-insensitive)
 *   3. Sender has an active session (.zyrex on) AND sent within
 *      the idle window of their/Zyrex's last exchange
 *
 * Storage:
 *   "zyrex_config"   -> array of { chatJid, active, dailyLimit, usedToday, resetAt }
 *   (per-group master switch + admin-set daily API-call cap)
 *
 * In-memory (NOT persisted — resets on restart, which is fine):
 *   zyrexSessions -> Map("chatJid_userJid" -> { active, lastMsgAt, history[] })
 *   zyrexCooldown -> Map("chatJid_userJid" -> lastReplyTimestamp)
 *
 * Safety guardrails (hard-coded into the system prompt, not
 * configurable via chat commands — these are load-bearing):
 *   - Never claims an age, especially a minor one
 *   - Never generates slurs, sexual content, or content
 *     sexualizing anyone
 *   - Never runs a real, targeted harassment/pile-on campaign
 *     against a named person — blunt/sarcastic banter is fine,
 *     sustained targeted abuse is not
 *   - Never claims to be human or denies being AI if directly
 *     and sincerely asked
 *   - NEVER triggers real admin actions (kick/warn/mute) from
 *     its own conversational output — that stays entirely
 *     separate, only through the bot's existing explicit
 *     admin commands
 *
 * Paste at the bottom of cmds/group.js
 * Additional imports needed beyond what group.js already has:
 *   node-fetch (for Groq API calls)
 *   wa-sticker-formatter (for mood stickers)
 * Both are already dependencies elsewhere in this repo (dl.js
 * imports wa-sticker-formatter; several files use node-fetch).
 */
...
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
const ZX = {
  BOT_NAME: "zyrex",                 // name it listens for in messages
  SESSION_IDLE_MS: 90 * 1000,        // continuation window after last exchange
  COOLDOWN_MS: 4 * 1000,             // min gap between replies to the same user
  DEFAULT_DAILY_LIMIT: 300,          // per-group API calls/day, admin-adjustable
  HISTORY_TURNS: 10,                 // max stored turns per session (memory only)
  STICKER_CHANCE: 0.35,              // probability of attaching a sticker to a reply
  GROQ_MODEL: "llama-3.3-70b-versatile",
  GROQ_URL: "https://api.groq.com/openai/v1/chat/completions",
}

// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
//  STICKER MOOD MAP — paste your real sticker URLs (direct
//  .webp, or .png/.jpg — wa-sticker-formatter converts them)
//  into these arrays. One is picked at random per mood.
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
const ZYREX_STICKERS = {
  savage:      [ /* paste URLs here */ ],
  laughing:    [ /* paste URLs here */ ],
  done:        [ /* paste URLs here */ ], // "done with this conversation"
...
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
//  SESSION / COOLDOWN STATE (in-memory, per process)
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
async function zyrexCheckAndIncrementUsage(chatJid) {
  ...
  if (entry.usedToday >= limit) return { allowed: false, limitReached: true }
...
//  Tone/flavor is adjustable — the guardrail section at the
//  bottom is NOT meant to be relaxed.
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
function zyrexSystemPrompt(isOwner, senderName) {
...
The person currently messaging you is named "${senderName}"${isOwner ? " and is your owner/creator — you can still be sarcastic with them, but with a hint of underlying respect/loyalty, not the same as strangers" : ""}.
...
- You do not have the ability to kick, warn, mute, or otherwise moderate anyone — if asked to do these things, say you can't and that they should use the actual bot commands.
...
${c} status        - view current settings\`\`\``
    ...
    console.log("zyrex cmd error", e)
    return await m.sendErr(e)
  }
})

// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
//  LISTENER — decides whether to reply, then calls Groq
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
kord({ on: "all" }, async (m, text) => {
  try {
    ...
    // 1. Reply to Zyrex's own message
    ...
    // ── Daily cap ──
    const usage = await zyrexCheckAndIncrementUsage(chatJid)
    if (!usage.allowed) return // silently skip — don't spam "limit reached" messages

    // ── Build conversation context ──
    const session = zyrexTouchSession(chatJid, userJid)
    const isOwner = userJid === m.ownerJid
    ...
    const messages = [
      { role: "system", content: zyrexSystemPrompt(isOwner, senderName) },
      ...session.history,
    ]
    ...
    try {
      ...
      console.log("zyrex groq error", e)
      return // fail silently in-chat; don't spam group with error messages
    }
    if (!rawReply) return
