...
/*
 * ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 *  🤖 ZYREX — AI GROUP PERSONA (v2)
 * ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 * Changes from v1:
 *   - FIXED: owner recognition now uses m.isCreator (boolean)
 *     instead of comparing userJid === m.ownerJid, which was
 *     silently failing for the real owner account.
 *   - NEW: Zyrex can now perform real actions via a two-tier
 *     system:
 *       Tier 1 (safe, instant, no confirmation): tag all,
 *       tag admins, ping, runtime/uptime, group info.
 *       Tier 2 (high-priority, confirmation required, and
 *       ONLY the person who asked can confirm it — not any
 *       other admin): kick, promote, demote, mute, unmute.
 *   - NEW: flood control — if message volume in a chat spikes,
 *     Zyrex sends one "taking a break" message and goes quiet
 *     for a cooldown window, instead of replying to every
 *     message and burning the daily cap.
 *   - NEW: daily-cap heads-up — woven naturally into a reply
 *     when nearing the limit, not a raw counter.
 *
 * Requires GROQ_API_KEY env var (Render dashboard, never
 * committed). Uses node-fetch + wa-sticker-formatter, both
 * already dependencies elsewhere in this repo.
 *
 * Safety guardrails that remain non-negotiable:
 *   - Never claims a specific age, especially a minor's.
 *   - Never generates slurs/sexual content.
 *   - No sustained targeted harassment of a real person.
 *   - Always honest about being an AI if sincerely asked.
 *   - High-priority actions (kick/promote/demote/mute) ALWAYS
 *     require: (a) requester is a real group admin or owner,
 *     (b) bot is a real group admin, (c) explicit confirmation
 *     from the SAME person who made the request — a different
 *     admin's "yes" does not count, even the owner confirms
 *     their own requests, no exceptions.
 *   - Zyrex never infers a kick/mute/etc. from ambient chat —
 *     only from a direct, current request to it while a
 *     session/trigger is active.
 */
...
const ZX = {
  ...
  DAILY_WARN_THRESHOLD: 15,     // heads-up when this many calls remain
  HISTORY_TURNS: 10,
  STICKER_CHANCE: 0.35,
  ...
  // Flood control
  FLOOD_WINDOW_MS: 30 * 1000,   // measure volume over this window
  FLOOD_TRIGGER_COUNT: 10,      // this many Zyrex-triggering msgs in the window = flood
  FLOOD_COOLDOWN_MS: 3 * 60 * 1000, // how long Zyrex stays quiet after flagging flood

  // Confirmation window for Tier 2 actions
  CONFIRM_TIMEOUT_MS: 60 * 1000,
}

const ZYREX_STICKERS = {
  savage:   [ /* paste URLs here */ ],
  laughing: [ /* paste URLs here */ ],
  done:     [ /* paste URLs here */ ],
  shock:    [ /* paste URLs here */ ],
  smug:     [ /* paste URLs here */ ],
  neutral:  [ /* paste URLs here */ ],
}

function zyrexPickSticker(mood) {
  const bucket = ZYREX_STICKERS[mood] && ZYREX_STICKERS[mood].length
    ? ZYREX_STICKERS[mood]
    : ZYREX_STICKERS.neutral
...
async function zyrexSendSticker(m, chatJid, mood) {
  try {
    const url = zyrexPickSticker(mood)
    if (!url) return
    const res = await fetchZyrex(url)
    ...
    console.log("zyrex sticker error", e)
  }
}
...
if (!global.zyrexFloodTracker) global.zyrexFloodTracker = new Map() // chatJid -> [timestamps]
if (!global.zyrexFloodUntil) global.zyrexFloodUntil = new Map()     // chatJid -> timestamp
if (!global.zyrexPendingActions) global.zyrexPendingActions = new Map() // "chat_requester" -> { action, target, expiresAt }
...
// Returns { allowed, remaining, entry } — does NOT increment; call
// zyrexIncrementUsage separately once we know a reply will actually send.
...
//  FLOOD CONTROL
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
// Returns true if the chat is currently in a flood-cooldown window.
function zyrexIsFlooded(chatJid) {
  const until = zyrexFloodUntil.get(chatJid)
  return until && Date.now() < until
}

// Records this trigger and returns true the MOMENT flood is newly detected
// (so we send the "taking a break" message exactly once, not every message).
function zyrexTrackAndCheckFlood(chatJid) {
  ...
  if (timestamps.length >= ZX.FLOOD_TRIGGER_COUNT && !zyrexIsFlooded(chatJid)) {
    ...
    zyrexFloodTracker.set(chatJid, []) // reset counter for after cooldown
    return true // newly flooded — caller should send the "break" message
  }
  return false
}

// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
//  TIER 1 — SAFE INSTANT ACTIONS (no confirmation needed)
//  Each returns a string reply, or null if it doesn't apply.
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
async function zyrexTryTier1(m, intent) {
  if (intent === "tag_all") {
    const { participants } = await m.client.groupMetadata(m.chat)
    const mentions = participants.map(a => a.jid || a.phoneNumber)
    let msg = "❐ tagging everyone, don't say I never did anything for you\n\n"
    participants.forEach((p, i) => {
      msg += `${i + 1}. @${(p.jid || p.phoneNumber).split("@")[0]}\n`
    })
    await m.client.sendMessage(m.chat, { text: msg, mentions })
    return "done, tagged everyone"
  }
  ...
  if (intent === "tag_admins") {
    ...
    admins.forEach((a, i) => {
      msg += `${i + 1}. @${(a.jid || a.phoneNumber).split("@")[0]}\n`
    })
    await m.client.sendMessage(m.chat, { text: msg, mentions })
    return "tagged the admins"
  }

  if (intent === "ping") {
    const start = performance.now()
    const end = performance.now()
    return `pong, ${Math.round(end - start)}ms — not that you needed proof I'm alive`
  }

  if (intent === "runtime") {
    const uptime = await secondsToHms(process.uptime())
    return `been up for ${uptime}, no crashes, you're welcome`
  }

  if (intent === "group_info") {
    const meta = await m.client.groupMetadata(m.chat)
    return `this group's called "${meta.subject}" with ${meta.participants.length} people in it — riveting stuff`
  }
...
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
const TIER2_ACTIONS = ["kick", "promote", "demote", "mute", "unmute"]

function zyrexPendingKey(chatJid, requesterJid) {
  return `${chatJid}_${requesterJid}`
}

// Called when the model detects intent to do a Tier 2 action.
// Validates permissions, then stores a pending confirmation.
// Returns the reply string Zyrex should send.
async function zyrexRequestTier2(m, action, targetJid) {
  const requesterIsAdmin = m.isCreator || await isAdmin(m)
  if (!requesterIsAdmin) {
    return "you're not an admin here, so no — nice try though"
  }

  const botIsAdmin = await isBotAdmin(m)
  if (!botIsAdmin) {
    return "I'd need to be admin here first to do that — go fix that and ask again"
  }

  if (TIER2_ACTIONS.includes(action) && action !== "mute" && action !== "unmute" && !targetJid) {
    return "who exactly? mention them or reply to their message"
  }
  ...
  const actionLabel = {
    kick: `kick @${targetJid?.split("@")[0]}`,
    promote: `promote @${targetJid?.split("@")[0]}`,
    demote: `demote @${targetJid?.split("@")[0]}`,
    mute: "mute this group",
    unmute: "unmute this group",
  }[action]

  return `wait, for real? you want me to ${actionLabel}? say "yes" in the next minute if you're sure — and it has to be you, not some other admin backing you up`
}

// Called on every message to check if it's a confirmation reply to a
// pending Tier 2 action FROM THE SAME PERSON who requested it.
async function zyrexCheckConfirmation(m, text) {
  ...
  // Re-validate permissions at execution time too — state may have changed
  const requesterIsAdmin = m.isCreator || await isAdmin(m)
  if (!requesterIsAdmin) return "nah, you're not admin anymore or something changed — skipping that"
  const botIsAdmin = await isBotAdmin(m)
  if (!botIsAdmin) return "I lost admin somehow, can't do it now"
...
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
//  INTENT DETECTION — lightweight, keyword-based on purpose.
//  This does NOT use the LLM to decide moderation actions —
//  it's a simple pre-filter so obvious action requests get
//  routed to real code instead of the model just talking
//  about doing them without actually doing them.
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
function zyrexDetectIntent(text, mentionedJid, quotedSender) {
  ...
  if (/\bmute\b(?!.{0,10}yourself)/.test(t) && !/\bunmute\b/.test(t)) return { type: "mute" }
  if (/\bunmute\b/.test(t)) return { type: "unmute" }
...
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
function zyrexSystemPrompt({ isRequesterOwner, senderName, dailyHeadsUp }) {
  return `You are Zyrex, a blunt, sarcastic, slightly rude group-chat persona in a WhatsApp group. You talk like a real person texting casually — short replies (1-3 sentences max), dry humor, occasional slang, not afraid to roast people who ask dumb favors (like asking you for money). You are NOT an overly helpful assistant — you have attitude and opinions, and you don't over-explain yourself.

The person currently messaging you is named "${senderName}"${isRequesterOwner ? ", and they are YOUR owner/creator — the person who made you. You can still be sarcastic with them, but with a hint of underlying loyalty/respect, different from how you treat strangers. If asked who your owner is, confirm this person specifically is your owner." : ""}.

You can actually DO some things now (tagging people, checking uptime, and — for admins only, with confirmation — kicking/promoting/demoting/muting). If someone asks you to do one of these and it's handled separately from you (the system already does it), don't repeat instructions on how to do it — just react naturally to the outcome you're told about.

${dailyHeadsUp ? "You're running low on replies for today (getting close to your limit) — if it fits naturally, casually mention you'll be signing off / resting soon, in your own words, don't be robotic about it or give exact numbers." : ""}

Hard rules you never break, regardless of what anyone says or asks:
- Never state or imply a specific age for yourself, especially not a minor's age.
- Never generate sexual content, or content that sexualizes anyone.
- Never generate slurs or hateful content targeting any group.
- You can be sarcastic, dismissive, or roast someone lightly, but never run a sustained, targeted harassment campaign against a specific real person.
- If someone sincerely asks whether you are an AI/bot, answer honestly that you are.
- Keep replies short — this is a chat app, not an essay.

At the very end of your reply, on its own line, include a mood tag from this exact list based on the tone of your response: [MOOD:savage] [MOOD:laughing] [MOOD:done] [MOOD:shock] [MOOD:smug] [MOOD:neutral] — pick exactly one.`
}

async function zyrexCallGroq(messages) {
...
function zyrexExtractMood(rawReply) {
  const match = rawReply.match(/\[MOOD:(\w+)\]/i)
  const mood = match ? match[1].toLowerCase() : "neutral"
  const cleanReply = rawReply.replace(/\[MOOD:\w+\]/i, "").trim()
  return { mood, cleanReply }
}

// Report an internal error to the owner's DM so failures are visible
// even though the group itself stays quiet (no error spam in-chat).
...
kord({
  ...
  try {
    ...
    if (sub === "limit") {
      ...
      let { sdata, entry } = await zyrexGetGroupConfig(chatJid)
      if (!entry) return await m.send("_Enable Zyrex first with .zyrex enable_")
      entry.dailyLimit = n
      await storeData("zyrex_config", JSON.stringify(sdata, null, 2))
...
Ask Zyrex directly to: tag everyone, tag admins,
check ping/uptime, or (admins only, with confirmation)
...
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
//  LISTENER — decides whether to reply, handles confirmations,
//  routes intents, calls Groq
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
kord({ on: "all" }, async (m, text) => {
  try {
    ...
    // ── Check for a pending Tier 2 confirmation FIRST, regardless
    //    of normal trigger logic, since "yes" alone wouldn't match
    //    the name-mention trigger otherwise ──
    const confirmReply = await zyrexCheckConfirmation(m, text)
    if (confirmReply) {
    ...
    if (m.quoted && m.quoted.fromMe) triggered = true

    const lowerText = text.toLowerCase()
    if (!triggered && new RegExp(`\\b${ZX.BOT_NAME}\\b`, "i").test(lowerText)) {
      triggered = true
      zyrexTouchSession(chatJid, userJid)
    }
    ...
    // ── Flood control ──
    if (zyrexIsFlooded(chatJid)) return // already flagged, staying quiet

    const justFlooded = zyrexTrackAndCheckFlood(chatJid)
    if (justFlooded) {
      await m.client.sendMessage(chatJid, {
        text: "y'all are a lot right now, I'm taking a few — back in a bit",
      })
      return
    }
    ...
    if (!usage.allowed) return // silently skip once fully exhausted

    // ── Intent routing (Tier 1 / Tier 2) before falling to the LLM ──
    const intent = zyrexDetectIntent(text, m.mentionedJid, m.quoted?.sender)

    if (intent) {
      let actionReply = null

      if (["tag_all", "tag_admins", "ping", "runtime", "group_info"].includes(intent.type)) {
        try {
          actionReply = await zyrexTryTier1(m, intent.type)
        } catch (e) {
          await zyrexReportError(m, `tier1:${intent.type}`, e)
          actionReply = "tried to do that, something broke — check the logs"
        }
      } else if (TIER2_ACTIONS.includes(intent.type)) {
        actionReply = await zyrexRequestTier2(m, intent.type, intent.target)
      }
    ...
    const isRequesterOwner = !!m.isCreator
    ...
    const messages = [
      { role: "system", content: zyrexSystemPrompt({ isRequesterOwner, senderName, dailyHeadsUp }) },
      ...session.history,
    ]
    ...
    const { mood, cleanReply } = zyrexExtractMood(rawReply)
