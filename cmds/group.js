...
/*
 * ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 *  🤖 ZYREX — AI GROUP PERSONA (v4)
 * ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 * Changes from v3:
 *   - REVERTED (safety): honest AI disclosure restored.
 *     If sincerely asked, Zyrex confirms it's an AI. It can
 *     still deflect/banter about the question itself, but
 *     it never insists it's human/"more than a bot" to
 *     someone genuinely asking. This is non-negotiable —
 *     see rationale in chat history if this file is edited
 *     again later.
 *   - Owner claim: no owner is claimed until a real, verified
 *     number is wired in. Zyrex says honestly it doesn't have
 *     a confirmed owner yet if asked, rather than claiming
 *     "Codex" to anyone who asks (which anyone could exploit
 *     by just asserting it in chat).
 *   - Garri: dialed back to an occasional preference, not a
 *     forced mention every reply.
 *   - NEW Tier 1 (instant, admin-checked, no confirmation):
 *       - per-user mute/unmute ("mute @user for 10m")
 *       - warn ("warn @user for spamming")
 *       - antilink / antiword / antispam voice toggles
 *         ("turn on antilink", "set antiword to kick")
 *       - "tell owner ..." relay (anyone can use this, no
 *         admin needed — it's a message relay, not an action)
 *
 * Tier 2 (admin + confirmation, requester-locked) unchanged:
 *   kick, promote, demote, group mute, group unmute.
 *
...
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
//  STICKERS — folder based: cmds/zyrex_stickers/<mood>/*
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
function zyrexSystemPrompt({ senderName, dailyHeadsUp, annoyanceLevel }) {
...
You're a guy. You have real (mild) preferences and opinions, you don't react the exact same way to everything. Garri is a food you genuinely like, but only bring it up when food/hunger genuinely comes up in conversation — not every message, that gets old fast.
...
Sticker packs: cmds/zyrex_stickers/<mood>/
just drop files in, no restart needed.\`\`\``
    )
  } catch (e) {
  ...
  try {
    ...
    if (intent) {
      ...
      if (TIER1_TYPES.includes(intent.type)) {
        ...
        try {
          actionReply = await zyrexTryTier1(m, intent.type, text)
        } catch (e) {
    ...
    const messages = [
      { role: "system", content: zyrexSystemPrompt({ senderName, dailyHeadsUp, annoyanceLevel: annoyance.count || 0 }) },
      ...session.history,
    ]
